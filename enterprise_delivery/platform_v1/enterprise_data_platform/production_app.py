...
from .durable import RelationalStore, SQLCatalog, SQLControlState, SQLPolicyEngine, SQLRegistry, jobs, objects, now_iso
...
class SQLOperationsProvider:
    ...
    def snapshot(self):
        ...
        return {'generated_at': now_iso(), 'system_status': 'operational', 'environment': 'prod',
            'metrics': {'active_data_products': {'value': resource_counts.get('datasets', 0)}},
            'resources': resource_counts, 'jobs': job_counts}
...
def build_runtime():
    ...
    search = OpenSearchTransport(required('EDP_SEARCH_URL'), secrets.resolve(required('EDP_SEARCH_TOKEN_REF')))
    embeddings = OpenSearchTransport(required('EDP_EMBEDDING_URL'), secrets.resolve(required('EDP_EMBEDDING_TOKEN_REF')))
    embedder = HTTPEmbeddingProvider(embeddings, json.loads(required('EDP_EMBEDDING_PROFILES')))
    ...
    exporter = DurableExportBackend(queue, storage, int(os.getenv('EDP_EXPORT_RETENTION_SECONDS', '86400')))
    ...
    service = GovernedService(store=store, control=control, governor=ResourceGovernor(store, limits),
        ...
        structured=TelemetryProxy(router, 'source'), keyword=TelemetryProxy(OpenSearchBackend(search, 'keyword'), 'keyword'), vector=TelemetryProxy(OpenSearchBackend(search, 'vector'), 'vector'),
        embedder=TelemetryProxy(embedder, 'embedding'), chunks=TelemetryProxy(chunks, 'hydration'), exporter=exporter, guardrails=GuardrailEngine(load_rules),
